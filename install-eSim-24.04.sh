...
if (( ubuntu_major >= 24 )); then
    echo "Ubuntu $ubuntu_version detected (modern)."
    kicadppa="kicad/kicad-dev-nightly"
    target_version="10"
else
    echo "Older Ubuntu detected ($ubuntu_version)."
    ...
    if dpkg -s kicad &>/dev/null; then
        ...
        if [[ "$installed_version" != "$target_version" ]]; then
            ...
            read -p "Do you want to remove it and install KiCad $target_version? (yes/no): " response
            ...
            echo "KiCad $target_version is already installed."
            exit 0
        fi
    fi
...
if ! grep -q "^deb .*${kicadppa}" /etc/apt/sources.list /etc/apt/sources.list.d/* 2>/dev/null; then
    ...
    sudo add-apt-repository --yes "ppa:$kicadppa"
    sudo apt-get update
    
else
    echo "KiCad PPA already exists."
fi
...
# Detect KiCad config version
KICAD_VERSION=$(ls ~/.config/kicad 2>/dev/null | sort -V | tail -n 1)

if [ -z "$KICAD_VERSION" ]; then
    echo "No KiCad config found, creating one..."
    KICAD_VERSION="${target_version}.0"
    mkdir -p ~/.config/kicad/$KICAD_VERSION
fi

KICAD_CONFIG="$HOME/.config/kicad/$KICAD_VERSION"

echo "Using KiCad config: $KICAD_CONFIG"

# Copy symbol table
cp kicadLibrary/template/sym-lib-table "$KICAD_CONFIG/"
echo "Symbol table copied"

# Detect symbol directory
SYMBOL_DIR=$(find /usr/share -type d -path "*kicad*/symbols" 2>/dev/null | head -n 1)

if [ -z "$SYMBOL_DIR" ]; then
    echo "Symbol directory not found, creating fallback..."
    SYMBOL_DIR="/usr/share/kicad/symbols"
    sudo mkdir -p "$SYMBOL_DIR"
fi

# Copy symbols
sudo cp -r kicadLibrary/eSim-symbols/* "$SYMBOL_DIR/"

# Cleanup
rm -rf kicadLibrary

# Fix ownership
sudo chown -R $USER:$USER "$SYMBOL_DIR"
